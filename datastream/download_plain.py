...
#
# Code modified by Florian Mahner (01-05-2018)
# fmahner@uos.de
...
global_dir = '/Users/florianmahner/Desktop/Donders_Internship/Programming/iVGAN/extra/'
glob_count = 1
...
def parse_and_sched(dl_dir='videos', num_threads=4):
    ...
    for d_set in d_sets:
        ...
        check_call(['/usr/local/bin/wget', web_host + d_set + '.csv.gz']) # Changed the wget location
        
        print (d_set + ': Unzipping annotations...')
        try:
            check_call(['/usr/bin/gzip', '-d', '-f', d_set + '.csv.gz']) # Change gzip location
        except:
            pass
        """
        ...
        with open((d_set + '.csv'), 'rt') as f: # Change this because not downloaded into parent directory
            reader = csv.reader(f)
