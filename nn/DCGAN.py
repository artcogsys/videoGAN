...
class Generator(VideoGAN):
    ...
    def __init__(self, ch = 512, **kwargs):
        ...
        with self.init_scope():
            ...
            self.deconv1 = convolution3d(ch, ch // 2, self.w, direction = 'backward', pad=1)
            self.deconv2 = convolution3d(ch // 2, ch // 4, self.w, direction = 'backward', pad=1)
            self.deconv3 = convolution3d(ch // 4, ch // 8, self.w, direction = 'backward', pad=1)
            self.deconv4 = convolution3d(ch // 8, 3, self.w, direction = 'backward', pad=1)
